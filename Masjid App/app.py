...
# If file doesn't exist, you could optionally create one, but here we assume it exists.
...
# You can now use the mosques dict and the location field in your app.
# For example, in your map popups you might use data["location"] instead of calling geolocator.reverse() again.
...
def create_map(user_location, tile_choice, filtered_mosques, distances, route=None, target_mosque=None):
    ...
    for name, data in filtered_mosques.items():
        # Use the pre-cached location from the JSON file
    ...
    # (The rest of the create_map code remains unchanged.)
...
def index():
    ...
    # Get Direction: Fetch route from OSRM
    route = None
    if action == "get_direction" and chosen_mosque:
        data = filtered_mosques[chosen_mosque]
        start_lon, start_lat = user_lon, user_lat  # Ensure correct format
        end_lon, end_lat = data["coords"][1], data["coords"][0]  # Ensure correct format
        ...
        if response.status_code == 200:
            try:
                ...
                if "routes" in route_data and route_data["routes"]:
                    route = [[coord[1], coord[0]] for coord in route_data["routes"][0]["geometry"]["coordinates"]]
    ...
    # Build the Folium map (pass chosen_mosque as target_mosque)
